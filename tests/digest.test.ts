...
async function addSearch(db: Db, name: string, criteria: SavedSearchCriteria): Promise<string> {
  const id = newId("srch");
  const now = nowIso();
  await db
...
describe("computeDigest", () => {
  ...
  it("reports new matches once, then nothing on re-run (idempotent)", async () => {
    await addSearch(db, "cheap 1BRs", { maxPrice: 3200, maxBedrooms: 1 });
    const first = await computeDigest(db, { dryRun: false });
    expect(first.totalNewMatches).toBe(2); // a1 (Mission) + a3 (Marina), a2 too pricey
    const second = await computeDigest(db, { dryRun: false });
    expect(second.totalNewMatches).toBe(0);
  });

  it("dry-run does not mark matches notified", async () => {
    await addSearch(db, "cheap 1BRs", { maxPrice: 3200, maxBedrooms: 1 });
    expect((await computeDigest(db, { dryRun: true })).totalNewMatches).toBe(2);
    expect((await computeDigest(db, { dryRun: true })).totalNewMatches).toBe(2);
    // Still nothing recorded as notified.
    const rows = await db.select().from(savedSearchMatches).all();
    expect(rows.every((r) => r.notifiedAt == null)).toBe(true);
  });

  it("detects price drops on already-notified matches only", async () => {
    const sid = await addSearch(db, "cheap 1BRs", { maxPrice: 3200, maxBedrooms: 1 });
    await computeDigest(db, { dryRun: false }); // notifies a1@3000, a3@2800
  ...
  it("reports matches that stop matching as dropped-out, once", async () => {
    await addSearch(db, "cheap 1BRs", { maxPrice: 3200, maxBedrooms: 1 });
    await computeDigest(db, { dryRun: false });
    ...
    expect((await computeDigest(db, { dryRun: false })).totalDroppedOut).toBe(0);
  });

  it("excludes user-hidden and likely-unavailable listings from matches", async () => {
    await addSearch(db, "cheap 1BRs", { maxPrice: 3200, maxBedrooms: 1 });
  ...
  it("surfaces unknown fields on matches instead of hiding them", async () => {
    // Dog-required search; stub listings have unknown pet policy.
    await addSearch(db, "dog friendly", { maxPrice: 3200, maxBedrooms: 1, requireDogsAllowed: true });
    const digest = await computeDigest(db, { dryRun: true });
  ...
  it("only evaluates enabled saved searches", async () => {
    const sid = await addSearch(db, "disabled search", { maxPrice: 10000 });
    await db.update(savedSearches).set({ enabled: false }).where(eq(savedSearches.id, sid)).run();
    expect((await computeDigest(db, { dryRun: true })).savedSearchesEvaluated).toBe(0);
  });
});
