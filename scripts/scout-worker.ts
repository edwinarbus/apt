...
/**
 * The local worker for Apt Scout's self-hosted environment. Keep this running
 ...
 *
 * Prereqs (from `npm run scout:deploy -- --deploy`):
 *   • APT_SCOUT_ENV_ID (or ANTHROPIC_ENVIRONMENT_ID) — the self-hosted env id
...
async function main() {
  ...
  const environmentId = process.env.ANTHROPIC_ENVIRONMENT_ID ?? process.env.APT_SCOUT_ENV_ID;
  ...
  if (!environmentKey || !environmentId) {
    console.error(
      "Missing env. The self-hosted worker needs:\n" +
        ...
        "  APT_SCOUT_ENV_ID           (printed by `npm run scout:deploy -- --deploy`)\n",
    );
    process.exit(1);
  }
  ...
  console.log(
    `Apt Scout worker polling ${environmentId}\n` +
      `  workdir: ${process.cwd()}\n` +
